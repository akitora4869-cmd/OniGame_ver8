...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private void setupTagPlayer(Player p,Location shrine,int index){
        common(p);p.getAttribute(Attribute.GENERIC_MAX_HEALTH).setBaseValue(10.0);p.setHealth(10.0);p.setFallDistance(0);p.getInventory().clear();
    ...
    private void startTag(CommandSender starter,boolean force){
        ...
        resetRuntime();oniType=selectedOniType!=null&&!isOniTypeLocked(selectedOniType)?selectedOniType:firstUnlockedOni();List<UUID> ids=new ArrayList<>(participants);Collections.shuffle(ids);
        UUID humanOni=null;if(ids.size()>1){if(forcedOni!=null&&ids.contains(forcedOni))humanOni=forcedOni;else for(UUID id:ids)if("oni".equals(rolePreferences.get(id))){humanOni=id;break;}if(humanOni==null&&!force)for(UUID id:ids)if(!"player".equals(rolePreferences.get(id))){humanOni=id;break;}}
        if(humanOni!=null){oni=humanOni;oniTeam.add(humanOni);ids.remove(humanOni);}players.addAll(ids);activePlayerSpawn=shrine;activeLobby=LocationStore.get(getConfig(),"locations.lobby");secondsLeft=Math.max(60,getConfig().getInt("tag-mode.game-seconds",1200));state=GameState.RUNNING;applyGameNight(shrine.getWorld());hideLobbyUtilities();setTagShopVisibility(true);Location tc=LocationStore.get(getConfig(),"tag-mode.center");if(tc!=null&&tc.getWorld()!=null){WorldBorder wb=tc.getWorld().getWorldBorder();wb.setCenter(tc.getX(),tc.getZ());wb.setSize(Math.max(16,getConfig().getDouble("tag-mode.border-size",200.0)));}
        int i=0;for(UUID id:players){Player p=Bukkit.getPlayer(id);if(p!=null)setupTagPlayer(p,shrine,i++);}int target=getPlayerBotTargetCount();int added=fillPlayerBotsToTarget(shrine,target);
        if(humanOni!=null){Player op=Bukkit.getPlayer(humanOni);if(op!=null){common(op);op.teleport(oniShrine);op.setGameMode(GameMode.SPECTATOR);msg(op,"&4鬼はまだ出現できない。 &7神社で待機中……");}}
        int delay=Math.max(0,getConfig().getInt("tag-mode.oni-release-seconds",45));all("&d&l鬼ごっこ開始！ &f神社から散開せよ。 &7鬼は一定時間後に現れる……");
        final UUID releaseHuman=humanOni;tagReleaseTask=Bukkit.getScheduler().runTaskLater(this,()->{if(state!=GameState.RUNNING||!isTagMode())return;if(releaseHuman!=null){Player op=Bukkit.getPlayer(releaseHuman);if(op!=null)setupOni(op,oniShrine);}else spawnOniBot(oniShrine);all("&4&l鬼が現れた――");for(UUID id:players){Player p=Bukkit.getPlayer(id);if(p!=null)p.playSound(p.getLocation(),Sound.ENTITY_WITHER_SPAWN,.75f,.65f);}},delay*20L);
    ...
    private void tickTag(){if(state!=GameState.RUNNING||!isTagMode())return;secondsLeft--;int fs=Math.max(0,getConfig().getInt("tag-mode.final-seconds",180));if(!finalPhase&&secondsLeft<=fs){finalPhase=true;tagParkourGrandfathered.addAll(tagParkourPlayers);all("&4&l【最終フェーズ】 &cこれ以降、倒れた者は復活できない。");}updateLateGameOniPower();if(secondsLeft<=0)end(false,"鬼ごっこ終了――生存者の勝利");}
    ...
    private double finalAttackMultiplier(){if(!finalPhase)return 1.0;int t=Math.max(0,secondsLeft);if(t<=30)return 1.80;if(t<=60)return 1.55;if(t<=90)return 1.35;if(t<=120)return 1.20;if(t<=150)return 1.10;return 1.0;}
    private double finalSpeedMultiplier(){if(!finalPhase)return 1.0;int t=Math.max(0,secondsLeft);if(t<=30)return 1.22;if(t<=60)return 1.17;if(t<=90)return 1.12;if(t<=120)return 1.08;if(t<=150)return 1.04;return 1.0;}
    private double finalCooldownMultiplier(){if(!finalPhase)return 1.0;int t=Math.max(0,secondsLeft);if(t<=30)return .40;if(t<=60)return .55;if(t<=90)return .70;if(t<=120)return .80;if(t<=150)return .90;return 1.0;}
    private boolean oniPowerActive(){return state==GameState.RUNNING&&secondsLeft<=600;}
    private double oniSkillEffectMultiplier(){if(!oniPowerActive())return 1.0;if(finalPhase){if(secondsLeft<=30)return 1.60;if(secondsLeft<=60)return 1.50;return 1.35;}return 1.20;}
    ...
    private void enforcePhaseTime(){
        ...
        if(state==GameState.WAITING)world.setTime(lobbyMorningTime());
            ...
            world.setTime(finalPhase ? finalSkyTime() : gameNightTime());
            if(finalPhase){
    ...
    private void useShikkiHuntingLeap(Player p){
        ...
        if(next<max)p.sendActionBar(cc("&6跳躍狩り &f"+next+"/&f"+max+" &7――空中で追加入力！ &8("+grace+"ms)"));else p.sendActionBar(cc("&c&l三連跳躍 &7――最大連鎖！ 命中で即時回復"));
    ...
    @EventHandler public void onDamage(EntityDamageByEntityEvent e){
        ...
        boolean attackerIsOni=(oni!=null&&isOni(rawAttacker.getUniqueId()))||attackerIsBot;if(attackerIsOni&&finalPhase)e.setDamage(e.getDamage()*finalAttackMultiplier());
